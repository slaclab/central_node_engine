...
class MitigationDeviceTest {
public:
  uint32_t mitigationId;
  uint32_t beamClassId;
  std::string name;
};

typedef shared_ptr<MitigationDeviceTest> MitigationDeviceTestPtr;
typedef std::map<uint32_t, MitigationDeviceTestPtr> MitigationDeviceTestMap;
typedef shared_ptr<MitigationDeviceTestMap> MitigationDeviceTestMapPtr;

class AnalogTest {
 public:
  uint32_t analogId;
  uint32_t value;
  MitigationDeviceTestMapPtr mitigationDevices;
};

typedef shared_ptr<AnalogTest> AnalogTestPtr;
typedef std::vector<AnalogTestPtr> AnalogTestVector;
typedef std::map<uint32_t, AnalogTestPtr> AnalogTestMap;
typedef shared_ptr<AnalogTestMap> AnalogTestMapPtr;
typedef shared_ptr<AnalogTestVector> AnalogTestVectorPtr;
...
namespace YAML {
   ...
   */
  template<>
    struct convert<AnalogTestVectorPtr> {
    static bool decode(const Node &node, AnalogTestVectorPtr &rhs) {
      AnalogTestVector *tests = new AnalogTestVector();
      rhs = AnalogTestVectorPtr(tests);

      for (YAML::Node::const_iterator it = node["AnalogTest"].begin();
	   it != node["AnalogTest"].end(); ++it) {
	AnalogTest *test = new AnalogTest();
	
	test->analogId = (*it)["analog_id"].as<int>();
	test->value = (*it)["value"].as<int>();

	rhs->push_back(AnalogTestPtr(test));
      }
...
class EngineTest {
 public:
  ...
  int loadAnalogFile(std::string inputFileName) {
...
int main(int argc, char **argv) {
  ...
  if (analogFileName != "") {
    ...
    if (t->loadAnalogFile(analogFileName)) {
