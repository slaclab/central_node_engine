# Rules for CENTRAL_NODE_ENGINE makefiles
...
sub-./%:
	$(QUIET)$(MAKE) $(QUIET:%=-s) -C  $(TARCH)  UPDIR=../$(UPDIR) \
         CENTRAL_NODE_ENGINE_DIR="$(call ADD_updir,$(CENTRAL_NODE_ENGINE_DIR),../)" \
...
# don't apply ADD_updir to central_node_enginelib_DIRS because CENTRAL_NODE_ENGINE_DIR already was 'upped'.
# This means that e.g. yaml_cpplib_DIR must be absolute or relative to CENTRAL_NODE_ENGINE_DIR
$(BINTGTS): LIBARGS += $(addprefix -L,$(subst :, ,$(central_node_enginelib_DIRS)))
...
libpath:
	@echo export LD_LIBRARY_PATH="$(subst $(SPACE__),:,$(patsubst ../%,%,$(subst :, ,$(central_node_enginelib_DIRS))))$${LD_LIBRARY_PATH:+:$$LD_LIBRARY_PATH}"
endif
